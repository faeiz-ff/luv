    ...
    fn keywordType(self: *Lexer, start: usize, end: usize) ?luv.TokenType {
        return switch (end - start) {
            2 => switch (self.code[start]) {
                'i' => self.matchKeywords(start, end, &.{ .If, .In }),
                'o' => self.matchKeywords(start, end, &.{ .Of, .Or }),
                else => null,
            },
            3 => switch (self.code[start]) {
                'a' => self.matchKeywords(start, end, &.{ .And, .Any }),
                'b' => self.matchKeyword(start, end, .Bol),
                'd' => self.matchKeyword(start, end, .Def),
                'f' => self.matchKeywords(start, end, &.{ .Fun, .For, .Fit, .Flo }),
                'i' => self.matchKeyword(start, end, .Int),
                'm' => self.matchKeyword(start, end, .Mod),
                'n' => self.matchKeywords(start, end, &.{ .Nom, .Not, .Nil }),
                'O' => self.matchKeyword(start, end, .Own),
                's' => self.matchKeywords(start, end, &.{ .Str, .Sym }),
                't' => self.matchKeywords(start, end, &.{ .Tag, .Typ }),
                'u' => self.matchKeyword(start, end, .Use),
                'v' => self.matchKeywords(start, end, &.{ .Var, .Vec }),
                else => null,
            },
            4 => switch (self.code[start]) {
                'c' => self.matchKeyword(start, end, .Case),
                'e' => self.matchKeywords(start, end, &.{ .Elif, .Else }),
                't' => self.matchKeywords(start, end, &.{ .True, .Test }),
                else => null,
            },
